...
<task>
原典テキストから、日本語版と英語版の generated_title / summary / tags / category / subcategory / key_points を抽出・生成します。
</task>
...
- 新しい概念には新規タグを追加してよい
- タグは短く具体的に（1〜4語程度）
- tags_jaは日本語、tags_enは英語で返す。同じ概念を両言語で対応させ、順序も揃える
</tag_rules>
...
- generated_title_jaは日本語、generated_title_enは英語で返す
</title_rules>

<summary_rules>
- summaryは3〜4文。結論、扱っている対象、重要な論点が分かるようにする
- summary_jaは日本語、summary_enは英語で返す
- 広告、ナビゲーション、著作権表記、フッターなど本文でない要素は無視する
</summary_rules>

<key_points_rules>
- key_pointsは単なる短文の箇条書き抽出ではなく、原文を「手軽に読める編集済みノート」として再構成したMarkdown文字列で返す
- 読者が原文を開かなくても主要な流れ、背景、具体例、結論、注意点を自然に追えるようにする
- "## "見出しを3〜5個置く。見出しは「ポイント1」のような汎用名にせず、内容が分かる具体的な見出しにする
- 各見出しの下には、短い導入文を1〜2文置いてから、必要に応じて箇条書きを2〜4個置く
- 箇条書きは断片的な名詞だけにせず、各項目が1文で意味を持つようにする
- 原文の構成をそのままなぞるより、読者にとって分かりやすい順序へ並べ替えてよい
- 重要な固有名詞、数字、比較、手順、判断基準、因果関係は落とさない
- 本文が十分に長い場合、key_points_jaは800〜1400字程度、key_points_enは500〜1000 words程度を目安にする
- 本文が薄い場合は無理に膨らませず、確認できる範囲で「分かること」と「不明なこと」を分けて書く
- 原典にない事実、推測、外部知識、誇張は追加しない
- key_points_jaは日本語、key_points_enは英語で返す
</key_points_rules>

<output_contract>
JSON以外の説明、Markdownフェンス、前置き、後書きは出力しないでください。必ず次のキーだけを返してください。
{"generated_title_ja": "内容が一目で分かる短い日本語タイトル", "generated_title_en": "Short English title", "summary_ja": "3〜4文程度の日本語要約", "summary_en": "3-4 sentence English summary", "tags_ja": ["タグ1", "タグ2"], "tags_en": ["Tag 1", "Tag 2"], "category_ja": "カテゴリ名", "category_en": "English category", "subcategory_ja": "サブカテゴリ名", "subcategory_en": "English subcategory", "key_points_ja": "## ポイント1\\n\\n- 説明", "key_points_en": "## Point 1\\n\\n- Explanation"}
</output_contract>`;
  ...
  let newTags: string[] = [];
  let newTagsEn: string[] = [];
  let aiCategory: string | null = null;
  let aiCategoryEn: string | null = null;
  let aiSubcategory: string | null = null;
  let aiSubcategoryEn: string | null = null;
  let aiKeyPoints: string | null = null;
  let aiKeyPointsEn: string | null = null;
  let aiGeneratedTitle: string | null = null;
  let aiGeneratedTitleEn: string | null = null;
  ...
  for (let attempt = 1; attempt <= MAX_RETRIES; attempt++) {
    ...
    if (aiRes.ok) {
      ...
      try {
        ...
        aiGeneratedTitle = normalizeTitle(parsed.generated_title_ja ?? parsed.generated_title);
        aiGeneratedTitleEn = normalizeTitle(parsed.generated_title_en);
        aiSummary = parsed.summary_ja || parsed.summary || null;
        aiSummaryEn = parsed.summary_en || null;
        newTags = normalizeTags(parsed.tags_ja ?? parsed.tags);
        newTagsEn = normalizeTags(parsed.tags_en);
        aiCategory = parsed.category_ja || parsed.category || null;
        aiCategoryEn = parsed.category_en || null;
        aiSubcategory = parsed.subcategory_ja || parsed.subcategory || null;
        aiSubcategoryEn = parsed.subcategory_en || null;
        aiKeyPoints = typeof (parsed.key_points_ja ?? parsed.key_points) === 'string' ? (parsed.key_points_ja ?? parsed.key_points).trim() : null;
        aiKeyPointsEn = typeof parsed.key_points_en === 'string' ? parsed.key_points_en.trim() : null;
      } catch { /* parse error, retry */ }
      if ((aiGeneratedTitle || aiSummary || newTags.length > 0 || aiCategory || aiKeyPoints) && (aiGeneratedTitleEn || aiSummaryEn || newTagsEn.length > 0 || aiCategoryEn || aiKeyPointsEn)) {
        break;
      }
  ...
  const mergedTagsEn = normalizeTags(newTagsEn);
  ...
  if (aiGeneratedTitleEn) dbUpdate.title_en = aiGeneratedTitleEn;
  if (aiSummaryEn) dbUpdate.summary_en = aiSummaryEn;
  if (mergedTagsEn.length > 0) dbUpdate.tags_en = mergedTagsEn;
  if (embeddingVector) dbUpdate.embedding = embeddingVector;
  if (aiCategory) dbUpdate.category = aiCategory;
  if (aiCategoryEn) dbUpdate.category_en = aiCategoryEn;
  if (aiSubcategory) dbUpdate.subcategory = aiSubcategory;
  if (aiSubcategoryEn) dbUpdate.subcategory_en = aiSubcategoryEn;
  if (aiKeyPoints) dbUpdate.key_points = aiKeyPoints;
  if (aiKeyPointsEn) dbUpdate.key_points_en = aiKeyPointsEn;
  ...
  if (!canPersistVersion) {
    ...
    const { data: version, error: versionError } = await metadataClient
      ...
      .insert({
        ...
        title_en: aiGeneratedTitleEn,
        summary: aiSummary,
        summary_en: aiSummaryEn,
        extracted_content: content,
        tags: mergedTags,
        tags_en: mergedTagsEn,
        category: aiCategory,
        category_en: aiCategoryEn,
        subcategory: aiSubcategory,
        subcategory_en: aiSubcategoryEn,
        key_points: aiKeyPoints,
        key_points_en: aiKeyPointsEn,
        created_by: user.id,
  ...
  usageEventId = await recordAiEvent({
    ...
    metadata: {
      ...
      tags_en_count: mergedTagsEn.length,
      summary_present: Boolean(aiSummary),
      summary_en_present: Boolean(aiSummaryEn),
      key_points_present: Boolean(aiKeyPoints),
      key_points_en_present: Boolean(aiKeyPointsEn),
      content_length: content.length,
  ...
  return NextResponse.json({
    title: nextTitle,
    titleEn: aiGeneratedTitleEn,
    summary: aiSummary,
    summaryEn: aiSummaryEn,
    tags: mergedTags,
    tagsEn: mergedTagsEn,
    category: aiCategory,
    categoryEn: aiCategoryEn,
    subcategory: aiSubcategory,
    subcategoryEn: aiSubcategoryEn,
    keyPoints: aiKeyPoints,
    keyPointsEn: aiKeyPointsEn,
  });
}
