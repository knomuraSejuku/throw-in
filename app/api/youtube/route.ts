...
const PIPED_STREAMS_URL = 'https://api.piped.private.coffee/streams';
...
async function fetchTranscriptFromTrack(track: CaptionTrack): Promise<TranscriptItem[]> {
  ...
  return fetchTranscriptFromTimedTextUrl(url);
}

async function fetchTranscriptFromTimedTextUrl(url: URL): Promise<TranscriptItem[]> {
  url.searchParams.set('fmt', 'json3');
...
async function fetchYouTubeTranscriptDirect(videoId: string) {
...
async function fetchYouTubeTranscriptViaPiped(videoId: string) {
  const response = await fetch(`${PIPED_STREAMS_URL}/${encodeURIComponent(videoId)}`, {
    headers: { Accept: 'application/json' },
  });

  if (!response.ok) {
    throw new Error(`Piped streams API failed: ${response.status}`);
  }

  const data = await response.json().catch(() => null) as PipedStreamResponse | null;
  const subtitles = data?.subtitles ?? [];
  const subtitle =
    subtitles.find(item => item.code === 'ja' && item.autoGenerated) ||
    subtitles.find(item => item.code === 'ja') ||
    subtitles[0];
  ...
  const transcript = await fetchTranscriptFromTimedTextUrl(new URL(subtitle.url));
  if (transcript.length > 0) return transcript;
...
async function fetchYouTubeTranscript(videoId: string) {
  try {
    ...
    try {
      return await fetchYouTubeTranscriptViaPiped(videoId);
    } catch {
      throw directError instanceof Error ? directError : new Error('YouTube transcript extraction failed');
    }
  }
}
